...
<script>
  ...
  export default {
    ...
    methods: {
      loginsiabbagor(){
        ...
        ServiceAPI.getPuppeterSiabbagor(data)
          .then((response) => {
            var halaman = response.data.response.data.halaman_siabbagor[0].halaman
            var koki = []
            ...
            ServiceAPI.insertCredential(datacredential).then((res) => {
              console.log(halaman)
                if(halaman == 'https://siabbagor.bondowosokab.go.id'){
                  this.loading = false
                }else{
                  this.loading = false
                  console.log(res)
                  window.location.href = "https://siabbagor.bondowosokab.go.id/dashboard"; 
                }
            })
    ...
    async beforeCreate() {
      ...
      this.loading = false
      this.load = false
      window.location.href = "https://sinobo.bondowosokab.go.id/tulis/proposal";
      // var datacari = {
      ...
      //                 .then((response) => {
      //                   console.log(response.data.response.data)
      //                   // var halaman = response.data.response.data.halaman_sibuba[0].halaman
      //                   var koki = []
      ...
      //                       document.cookie = koki[i].name + '=' + koki[i].value + '; expires='+sesi + '; path=/' + '; domain=.bondowosokab.go.id'
      //                   }
      //                   // ServiceAPI.insertCredential(datacredential).then((res) => {
      //                       // if(halaman == 'http://bondowosoku.bondowosokab.go.id/admin/login'){
      //                       //   this.loading = false
      //                       // }else{
      //                         this.loading = false
      //                         console.log(res)
      //                         window.location.href = "https://said.bondowosokab.go.id/info";
      //                       // }
      //                   // })
      //                   // window.location.href = "http://simpeg.bondowosokab.go.id/dashboard"; 
      //                 })
